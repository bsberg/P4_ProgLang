 ...
 */
void emitProcedurePrologue(DList instList,SymTable symtab, int index, int zip) {
	...
	//int offset = (int)SymGetFieldByIndex(symtab,zip,SYMTAB_OFFSET_FIELD);
	int offset = 2048;
	char offsetStr[10000];
	//char *inst;
	snprintf(offsetStr,9,"%d",offset); // was zip

	inst = nssave(3,"\tsubq $", offsetStr,", %rsp");
	dlinkAppend(instList,dlinkNodeAlloc(inst));

	// char * dec = (char*)malloc(200);
	// sprintf(dec, "        subq $%d, %%rsp", zip );
...
void emitDataPrologue(DList dataList) {
	...
	if (globalOffset != 0){
		printf("\t.comm _gp, %d, 4\n",globalOffset); 
		//sprintf(global,"%d",(globalOffset/4));
	}	                                              
...
void getKey(int k){
	if(k != 0){
		...
		global = k;
		//printf("%d", global);
		//snprintf(glob,9, "%d", global);
		//printf("Glob  %s\n", glob);
	}
	//snprintf(global,9,"%d",k); 
}

int keyStuff(SymTable symtab, SymTable symstack, char* key){
	//printf("Glob  %d\n", *global);
	int one;
	int two;
	//one = (int)SymGetFieldByIndex(symtab,varIndex,SYMTAB_OFFSET_FIELD);
 ...
 */
int emitComputeVariableAddress(DList instList, SymTable symtab, SymTable symStack, int varIndex, int key) {
	...
	one = (int)SymGetFieldByIndex(symtab,varIndex,SYMTAB_OFFSET_FIELD);
	two = (int)SymGetFieldByIndex(symStack,varIndex,SYMTAB_OFFSET_FIELD);
	//printf("one: %d two:%d \n", one, two);
	if( one == two && two != -1 ){
		isGlobal = 1;
	}
	...
	if(key){
		...
		int offset = (int)SymGetFieldByIndex(symStack,varIndex,SYMTAB_OFFSET_FIELD);
		char offsetStr[10];
		...
		//int regIndex2 = getFreeIntegerRegisterIndex(symStack);
		//char* regName2 = (char*)get64bitIntegerRegisterName(symStack, regIndex2);
		int offset2 = (int)SymGetFieldByIndex(symStack,varIndex,SYMTAB_OFFSET_FIELD);
		char offsetStr2[10];
		inst = nssave(2,"\tmovq %rbp, ",regName);
		offset2 = (offset2 * -1)+  (-4) ;
		//char *inst;
		snprintf(offsetStr2,9,"%d",offset2);
	    ...
	    //inst = nssave(2,"\tmovq $_gp,", regName); // original code 
		...
		dlinkAppend(instList,dlinkNodeAlloc(inst));
		inst = nssave(4,"\taddq $", offsetStr2, ", ", regName);
		dlinkAppend(instList,dlinkNodeAlloc(inst));
		//printf(" %s\n", offsetStr2);
		return regIndex;
	}
 ...
 * 		   array element.
 */
int emitComputeArrayAddress(DList instList, SymTable varSymtab, int varIndex, SymTable regSymtab, int subIndex, int key) {

	int isGlobal;
	int one;
	int two; 

	one = (int)SymGetFieldByIndex(regSymtab,varIndex,SYMTAB_OFFSET_FIELD);
	two = (int)SymGetFieldByIndex(varSymtab,varIndex,SYMTAB_OFFSET_FIELD);
	...
	if( one == two ){
		...
		//printf("%d\n", varTypeIndex);
		if (isArrayType(varSymtab,varTypeIndex)) { // changed reg to var
			...
			int size = (int)SymGetFieldByIndex(varSymtab,varIndex,SYMTAB_SIZE_FIELD);
			
			//int complete = (offset*size)*-1;
			//char finalStr[10];
			//snprintf(finalStr,9,"%d", complete);
			...
			//printf(">>%s\n", offsetStr);
			dlinkAppend(instList,dlinkNodeAlloc(inst));
			//inst = nssave(4,"\taddq $", offsetStr, ", ", regName); // original
			inst = nssave(4,"\taddq $", offsetStr , ", ", regName);
			dlinkAppend(instList,dlinkNodeAlloc(inst));
			...
			dlinkAppend(instList,dlinkNodeAlloc(inst));
				...
				char* regName2 = get64bitIntegerRegisterName(regSymtab, regIndex);
				int offset2 = (int)SymGetFieldByIndex(varSymtab,varIndex,SYMTAB_OFFSET_FIELD);
				int size2 = (int)SymGetFieldByIndex(varSymtab,varIndex,SYMTAB_SIZE_FIELD);
				...
				/* compute base address of array */
				// if (offset2 < 0){ 
				 	inst = nssave(2,"\tmovq %rbp, ",regName2);
				// 	//offset = offset*size;
				 	offset2 = ((offset2 *-1) + (-1 * size2));
				// }
				// else 
				//  	inst = nssave(2,"\tmovq $_gp, ", regName2);
				...
				dlinkAppend(instList,dlinkNodeAlloc(inst));
				//inst = nssave(4,"\taddq $", offsetStr, ", ", regName); // original
				inst = nssave(4,"\taddq $", offsetStr2 , ", ", regName2);
				dlinkAppend(instList,dlinkNodeAlloc(inst));

				/* compute offset based on subscript */
			    char* subReg32Name2 = (char*)SymGetFieldByIndex(regSymtab,subIndex,SYM_NAME_FIELD);
				char* subRegName2 = get64bitIntegerRegisterName(regSymtab, subIndex);
				
				inst = nssave(4,"\tmovslq ", subReg32Name2, ", ", subRegName2);
				dlinkAppend(instList,dlinkNodeAlloc(inst));

				inst = nssave(2,"\timulq $4, ", subRegName2);
				dlinkAppend(instList,dlinkNodeAlloc(inst));

				/* compute element address */
				inst = nssave(4,"\taddq ", subRegName2, ", ", regName2);
				dlinkAppend(instList,dlinkNodeAlloc(inst));
